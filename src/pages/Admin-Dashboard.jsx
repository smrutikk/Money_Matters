...
// import { FaTrash, FaEdit } from 'react-icons/fa'; // FaEdit and FaTrash are imported but not used. Consider removing if not planned.
...
const Admin_Dashboard = () => {
    ...
    datasets: [
      {
        label: 'Debit',
        data: [],
        backgroundColor: '#ef4444', // Using red for debit
      },
      {
        label: 'Credit',
        data: [],
        backgroundColor: '#22c55e', // Using green for credit
      },
    ],
  });
  ...
  const fetchDataInternal = async () => {
    ...
    try {
      const headers = {
        ...
        'x-hasura-admin-secret': 'g08A3qQy00y8yFDq3y6N1ZQnhOPOa4msdie5EtKS1hFStar01JzPKrtKEzYY2BtF', // Consider using environment variables
        'x-hasura-role': 'admin'
      };

      const [totalsRes, chartApiRes, transactionsRes] = await Promise.all([
        fetch('https://bursting-gelding-24.hasura.app/api/rest/transaction-totals-admin', { headers }),
        fetch('https://bursting-gelding-24.hasura.app/api/rest/daywise-totals-last-7-days-admin', { headers }),
        fetch(`https://bursting-gelding-24.hasura.app/api/rest/all-transactions?limit=4&offset=0`, { headers })
      ]);

      if (!totalsRes.ok) {
        const errorData = await totalsRes.json().catch(() => ({ message: 'Failed to parse error from totals API' }));
        throw new Error(`Totals API Error (${totalsRes.status}): ${errorData.message || totalsRes.statusText}`);
      }
      if (!chartApiRes.ok) {
        const errorData = await chartApiRes.json().catch(() => ({ message: 'Failed to parse error from chart API' }));
        throw new Error(`Chart API Error (${chartApiRes.status}): ${errorData.message || chartApiRes.statusText}`);
      }
      if (!transactionsRes.ok) {
        const errorData = await transactionsRes.json().catch(() => ({ message: 'Failed to parse error from transactions API' }));
        throw new Error(`Transactions API Error (${transactionsRes.status}): ${errorData.message || transactionsRes.statusText}`);
      }

      const totalsDataJSON = await totalsRes.json();
      const chartDataJSON = await chartApiRes.json();
      const transactionsDataJSON = await transactionsRes.json();

      console.log('DEBUG: Raw totalsData from API:', JSON.stringify(totalsDataJSON, null, 2));
      console.log('DEBUG: Raw chartData from API:', JSON.stringify(chartDataJSON, null, 2));
      console.log('DEBUG: Raw transactionsData from API:', JSON.stringify(transactionsDataJSON, null, 2));

      // --- Process Totals Data ---
      if (totalsDataJSON && totalsDataJSON.errors && Array.isArray(totalsDataJSON.errors) && totalsDataJSON.errors.length > 0) {
        console.error('DEBUG: Hasura API Error in totalsData:', totalsDataJSON.errors);
        throw new Error(`Totals API returned errors: ${totalsDataJSON.errors[0].message || 'Unknown Hasura error'}`);
      }
      
      const creditDebitTotalsFromAPI = totalsDataJSON?.transaction_totals_admin;

      if (!creditDebitTotalsFromAPI || !Array.isArray(creditDebitTotalsFromAPI)) {
        console.error('DEBUG: totalsData does not contain a valid "transaction_totals_admin" array. Actual totalsData:', totalsDataJSON);
        throw new Error('Invalid response structure from totals API. Expected "transaction_totals_admin" to be an array.');
      }
      
      const creditTotal = creditDebitTotalsFromAPI.find(t => t?.type === 'credit')?.sum || 0;
      const debitTotal = creditDebitTotalsFromAPI.find(t => t?.type === 'debit')?.sum || 0;

      // --- Process Chart Data ---
      if (chartDataJSON && chartDataJSON.errors && Array.isArray(chartDataJSON.errors) && chartDataJSON.errors.length > 0) {
          console.error('DEBUG: Hasura API Error in chartData:', chartDataJSON.errors);
          throw new Error(`Chart API returned errors: ${chartDataJSON.errors[0].message || 'Unknown Hasura error'}`);
      }

      // CORRECTED ACCESS: The API returns data under 'last_7_days_transactions_totals_admin'
      const apiChartData = chartDataJSON?.last_7_days_transactions_totals_admin;

      if (Array.isArray(apiChartData) && apiChartData.length > 0) {
        const groupedByDate = apiChartData.reduce((acc, transaction) => {
          if (!transaction || typeof transaction.date !== 'string' || typeof transaction.sum !== 'number' || typeof transaction.type !== 'string') {
            console.warn('Skipping invalid chart transaction item:', transaction);
            return acc;
          }
        ...
        const last7DaysOfData = Object.values(groupedByDate)
          .sort((a, b) => new Date(a.date) - new Date(b.date))
          .slice(-7);

        const labels = last7DaysOfData.map(day => 
          new Date(day.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
        );
        const debitData = last7DaysOfData.map(day => day.debit);
        const creditData = last7DaysOfData.map(day => day.credit);

        setChartData({
          labels,
          datasets: [
            { label: 'Debit', data: debitData, backgroundColor: '#ef4444' },
            { label: 'Credit', data: creditData, backgroundColor: '#22c55e' }
          ]
        });
      } else {
          if (apiChartData && !Array.isArray(apiChartData)) {
            console.warn('DEBUG: Chart data (last_7_days_transactions_totals_admin) received from API was not an array. Actual data:', apiChartData);
          } else if (!apiChartData || (Array.isArray(apiChartData) && apiChartData.length === 0)) {
            console.log('DEBUG: No chart data available from API or data array is empty for last_7_days_transactions_totals_admin.');
          }
          setChartData({
              labels: [],
              datasets: [
                  { label: 'Debit', data: [], backgroundColor: '#ef4444' },
                  { label: 'Credit', data: [], backgroundColor: '#22c55e' }
              ]
          });
      }

      // --- Process Recent Transactions ---
      if (transactionsDataJSON && transactionsDataJSON.errors && Array.isArray(transactionsDataJSON.errors) && transactionsDataJSON.errors.length > 0) {
          console.error('DEBUG: Hasura API Error in transactionsData:', transactionsDataJSON.errors);
          throw new Error(`Transactions List API returned errors: ${transactionsDataJSON.errors[0].message || 'Unknown Hasura error'}`);
      }
      
      const recentTransactionsFromAPI = transactionsDataJSON?.transactions; // API directly returns 'transactions' key

      const recentTxArray = Array.isArray(recentTransactionsFromAPI) ? recentTransactionsFromAPI : [];
       if (!Array.isArray(recentTransactionsFromAPI)) { // This check is good for robustness
          console.warn('DEBUG: Recent transactions data (transactions) is not an array. Defaulting to empty. Actual transactionsData:', transactionsDataJSON);
      }

      const recent = recentTxArray
        .slice(0, 4)
        .map(tx => ({
          id: tx.id,
          name: tx.transaction_name || 'N/A',
      ...
      console.error('Fetch error in Admin_Dashboard (fetchDataInternal):', err);
  ...
  }, [user]); // Assuming fetchDataInternal does not change, or add it as a dependency if it uses props/state not covered by user
      ...
      <div className="flex-1 ml-64 min-h-screen bg-gray-50">
        <div className="flex justify-between items-center mb-6 bg-white p-4 shadow">
          ...
          {/*<button 
            ...
            className="bg-emerald-600 text-white px-4 py-2 rounded-2xl hover:bg-emerald-700 transition-colors"
          >
            + Add Transaction
          </button>
          <AddTransaction
            ...
            onTransactionAdded={() => {
              ...
              // Consider calling refreshData() instead of window.location.reload()
              // for a smoother UX, unless a full reload is necessary.
              // refreshData(); 
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M69.2312 27.6474C69.5075 25.4706 69.1522 24.7892 68.1562 25.6007C68.2931 25.8781 68.3001 26.2299 68.1759 26.6548C67.6999 27.4314 67.5908 28.287 67.8474 29.2146C68.3779 29.0509 68.8399 28.5262 69.2312 27.6474Z" fill="#FCB88D"/>
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {stats.recentTransactions.length > 0 ? (
                stats.recentTransactions.map((transaction) => (
                  <div key={transaction.id} className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {/* Icons are still commented out, if you plan to use them, uncomment and ensure react-icons is installed
                ...
                <p className="text-gray-500 col-span-full">No recent transactions found.</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow mb-6">
            ...
            <div> {/* Ensure this div has appropriate styling if chart needs specific dimensions, though responsive=true helps */}
              {chartData.labels.length > 0 ? (
                <Bar data={chartData} options={options} />
              ) : (
                <p className="text-gray-500 text-center py-4">No data available for the chart.</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow">
            ...
            {/* Example of displaying user ID if available and needed */}
